...
const projectRoot = path.resolve(process.env.BOOKCABIN_PROJECT_ROOT ?? path.join(currentDirPath, "..", ".."));
const docsCatalog = {
    readme: {
...
const server = new McpServer({
    ...
    instructions: "Use this server as shared project context for Bookcabin. Read overview and known gaps first. Treat all tools as read-only and never assume secrets are available.",
});
...
function parseWorkflowSummary(source, filePath) {
    ...
    return {
        ...
        trigger: onMatch?.[1]?.trim().replace(/^['"]|['"]$/g, "") ??
...
async function execAwsJson(serviceArgs) {
...
async function execAwsText(serviceArgs) {
    const args = [...buildAwsBaseArgs(), ...serviceArgs, "--output", "text"];
    const { stdout } = await execFileAsync("aws", args, { cwd: projectRoot, maxBuffer: 1024 * 1024 * 4 });
    return stdout.trim();
}
function makeAwsErrorResult(toolName, error) {
...
function makeGitHubErrorResult(toolName, error) {
    const message = error instanceof Error ? error.message : String(error);
    return makeTextResult(`${toolName} failed: ${message}`, {
        ok: false,
        tool: toolName,
...
async function probeHttpEndpoint(url, method, timeoutSeconds) {
...
async function fetchGitHubApi(pathname) {
...
function ec2StateRank(stateName) {
    switch (stateName) {
        case "running":
            return 0;
        case "pending":
...
async function runSsmDiagnosticCommand(instanceId, commandId, timeoutSeconds) {
    ...
    while (Date.now() - startedAt < timeoutSeconds * 1000) {
        await sleep(3000);
        const response = await execAwsJson(["ssm", "list-command-invocations", "--command-id", awsCommandId, "--details"]);
        ...
        if (invocation) {
            invocationResult = {
                ...
                StandardOutputContent: invocation.StandardOutputContent ?? invocation.CommandPlugins?.map((plugin) => plugin.Output ?? "").join("\n"),
...
async function runSsmRemediationCommand(instanceId, actionId, timeoutSeconds) {
...
function parseRouteFile(source, routeFile) {
...
server.tool("search_docs", {
    ...
    const text = hits
        .map((hit) => `- ${String(hit.title)} [line ${String(hit.line)}]: ${String(hit.snippet)} (${String(hit.uri)})`)
        .join("\n");
...
server.tool("health_check_services", {}, async () => {
    ...
    return makeTextResult(text, result);
});
server.tool("get_context_document", {
    document: z.enum(Object.keys(docsCatalog)),
}, async ({ document }) => {
...
server.tool("aws_environment_status", {}, async () => {
    try {
        const [callerIdentity, ssmInfo, awsVersion] = await Promise.all([
            execAwsJson(["sts", "get-caller-identity"]),
            execAwsJson(["ssm", "describe-instance-information", "--max-results", "10"]),
            execAwsText(["--version"]),
        ]);
        const managedInstances = ssmInfo.InstanceInformationList?.map((instance) => ({
        ...
        const text = [
            ...
            ...(managedInstances.length > 0
                ? managedInstances.map((instance) => `- ${instance.instanceId}: ${instance.pingStatus} (${instance.platform || "platform unknown"})`)
...
server.tool("aws_list_ec2_instances", {
    ...
    try {
        ...
        const text = filtered.length > 0
            ? filtered
                .map((instance) => `- ${instance.instanceId} (${instance.state}) ${instance.instanceType} ${instance.name ? `[${instance.name}] ` : ""}private=${instance.privateIp ?? "-"} public=${instance.publicIp ?? "-"}`)
                .join("\n")
...
server.tool("aws_list_ssm_managed_instances", {
    max_results: z.number().int().min(1).max(50).default(20),
}, async ({ max_results }) => {
    try {
        const response = await execAwsJson(["ssm", "describe-instance-information", "--max-results", String(max_results)]);
        ...
        const text = instances.length > 0
            ? instances
                .map((instance) => `- ${instance.instanceId}: ${instance.pingStatus} (${instance.platform || "platform unknown"})`)
                .join("\n")
...
server.tool("aws_list_ecr_repositories", {
    max_results: z.number().int().min(1).max(50).default(20),
}, async ({ max_results }) => {
    try {
        const response = await execAwsJson(["ecr", "describe-repositories", "--max-items", String(max_results)]);
        const repositories = response.repositories?.map((repository) => ({
        ...
        const text = repositories.length > 0
...
server.tool("aws_list_load_balancers", {
    max_results: z.number().int().min(1).max(50).default(20),
}, async ({ max_results }) => {
    try {
        const response = await execAwsJson(["elbv2", "describe-load-balancers", "--page-size", String(Math.min(max_results, 20))]);
        const loadBalancers = response.LoadBalancers?.slice(0, max_results).map((loadBalancer) => ({
        ...
        const text = loadBalancers.length > 0
            ? loadBalancers
                .map((loadBalancer) => `- ${loadBalancer.name} (${loadBalancer.type}/${loadBalancer.scheme}) state=${loadBalancer.state} dns=${loadBalancer.dnsName}`)
                .join("\n")
...
server.tool("aws_list_cloudwatch_log_groups", {
    ...
    try {
        ...
        const response = await execAwsJson(args);
        ...
        const text = logGroups.length > 0
            ? logGroups
                .map((group) => `- ${group.name} retention=${group.retentionInDays ?? "never-expire"} storedBytes=${group.storedBytes}`)
                .join("\n")
...
server.tool("aws_list_cloudwatch_log_streams", {
    ...
    try {
        ...
        const logStreams = response.logStreams?.map((stream) => ({
        ...
        const text = logStreams.length > 0
            ? logStreams
                .map((stream) => `- ${stream.name} lastEvent=${stream.lastEventAt ?? "-"} storedBytes=${stream.storedBytes}`)
                .join("\n")
...
server.tool("aws_tail_cloudwatch_logs", {
    log_group_name: z.string().min(1),
    ...
    try {
        ...
        const text = events.length > 0
            ? events
                .map((event) => `[${event.timestamp ?? "-"}] ${event.logStreamName}: ${event.message || "<empty message>"}`)
                .join("\n")
...
server.tool("aws_list_ssm_parameters", {
    ...
    try {
        ...
        const parameters = response.Parameters?.map((parameter) => ({
        ...
        const text = parameters.length > 0
...
server.tool("aws_list_cloudfront_distributions", {
    max_results: z.number().int().min(1).max(50).default(20),
}, async ({ max_results }) => {
    try {
        const response = await execAwsJson(["cloudfront", "list-distributions"]);
        const distributions = response.DistributionList?.Items?.slice(0, max_results).map((distribution) => ({
        ...
        const text = distributions.length > 0
            ? distributions
                .map((distribution) => `- ${distribution.id} (${distribution.status}) enabled=${distribution.enabled} domain=${distribution.domainName}`)
                .join("\n")
...
server.tool("aws_list_rds_instances", {
    include_stopped: z.boolean().default(true),
    max_results: z.number().int().min(1).max(50).default(20),
}, async ({ include_stopped, max_results }) => {
    try {
        const response = await execAwsJson(["rds", "describe-db-instances"]);
        ...
        const text = instances.length > 0
            ? instances
                .map((instance) => `- ${instance.identifier} (${instance.status}) ${instance.engine}@${instance.engineVersion} endpoint=${instance.endpoint ?? "-"}`)
                .join("\n")
...
server.tool("aws_list_target_groups", {
    max_results: z.number().int().min(1).max(50).default(20),
}, async ({ max_results }) => {
    try {
        const response = await execAwsJson(["elbv2", "describe-target-groups"]);
        const targetGroups = response.TargetGroups?.slice(0, max_results).map((targetGroup) => ({
        ...
        const text = targetGroups.length > 0
            ? targetGroups
                .map((targetGroup) => `- ${targetGroup.name} ${targetGroup.protocol}:${targetGroup.port ?? "-"} health=${targetGroup.healthCheckPath ?? "-"} matcher=${targetGroup.matcher ?? "-"}`)
                .join("\n")
...
server.tool("aws_get_target_group_health", {
    target_group_arn: z.string().optional(),
}, async ({ target_group_arn }) => {
    try {
        let targetGroupArn = target_group_arn;
        if (!targetGroupArn) {
            const targetGroupsResponse = await execAwsJson(["elbv2", "describe-target-groups"]);
        ...
        const response = await execAwsJson(["elbv2", "describe-target-health", "--target-group-arn", targetGroupArn]);
        const targets = response.TargetHealthDescriptions?.map((target) => ({
        ...
        const text = targets.length > 0
            ? targets
                .map((target) => `- ${target.targetId}:${target.port ?? "-"} state=${target.state}${target.reason ? ` reason=${target.reason}` : ""}`)
                .join("\n")
...
server.tool("aws_run_ssm_diagnostic_command", {
    instance_id: z.string().min(4),
    command_id: z.enum(Object.keys(ssmDiagnosticCommands)),
    ...
    try {
        ...
        return makeTextResult(textParts.join("\n"), {
            ...
            guardrail: "This tool only runs commands from a fixed read-only diagnostic allowlist and is intended for observability.",
        });
    }
    catch (error) {
...
server.tool("aws_probe_http_endpoint", {
    url: z.string().url(),
...
server.tool("aws_list_cloudwatch_metrics", {
    ...
    try {
        ...
        const response = await execAwsJson(args);
        const metrics = response.Metrics?.slice(0, max_results).map((metric) => ({
        ...
        const text = metrics.length > 0
            ? metrics
                .map((metric) => `- ${metric.metricName} (${metric.dimensions.map((dimension) => `${dimension.name}=${dimension.value}`).join(", ") || "no dimensions"})`)
                .join("\n")
...
server.tool("aws_get_cloudwatch_metric_series", {
    namespace: z.string(),
    ...
    try {
        ...
        const text = datapoints.length > 0
            ? datapoints
...
server.tool("github_actions_workflow_catalog", {}, async () => {
    try {
        ...
        const workflows = await Promise.all(workflowFiles.map(async (filePath) => parseWorkflowSummary(await readProjectFile(filePath), filePath)));
        const textParts = [
            `GitHub CLI available: no`,
            `Workflows found: ${workflows.length}`,
            ...workflows.map((workflow) => `- ${workflow.name} (${workflow.filePath}) trigger=${workflow.trigger} jobs=${workflow.jobs.join(", ") || "unknown"}`),
        ];
        return makeTextResult(textParts.join("\n"), {
...
server.tool("github_actions_recent_runs", {
    ...
    try {
        ...
        const text = [
            ...
            `Source: ${source}`,
            ...runs.map((run) => `- ${String(run.workflowName)} status=${String(run.status)} conclusion=${String(run.conclusion ?? "-")} branch=${String(run.headBranch ?? "-")} sha=${String(run.headSha ?? "").slice(0, 12)}`),
        ].join("\n");
        return makeTextResult(text, {
            ghCliAvailable: ghAvailable,
            remoteUrl,
            ...
            source,
            runs,
            note: source === "github-api" && !githubToken
...
server.tool("github_actions_find_run_for_sha", {
    sha: z.string().min(7),
    workflow_file: z.string().optional(),
    ...
    try {
        ...
        const text = [
            ...
            `Source: ${source}`,
            ...(runs.length > 0
                ? runs.map((run) => `- ${String(run.workflowName)} status=${String(run.status)} conclusion=${String(run.conclusion ?? "-")} sha=${String(run.headSha ?? "").slice(0, 12)}`)
...
server.tool("aws_list_ecr_image_tags", {
    repository_name: z.string().min(1),
    max_results: z.number().int().min(1).max(50).default(20),
}, async ({ repository_name, max_results }) => {
    try {
        const response = await execAwsJson(["ecr", "describe-images", "--repository-name", repository_name, "--max-items", String(max_results)]);
        ...
        const text = images.length > 0
            ? images
                .map((image) => `- ${image.tags.join(", ") || "<untagged>"} digest=${image.digest} pushedAt=${image.pushedAt ?? "-"}`)
                .join("\n")
...
server.tool("aws_detect_ec2_container_drift", {
    instance_id: z.string().min(4),
    ...
    try {
        const [reposResponse, runtime] = await Promise.all([
            execAwsJson(["ecr", "describe-repositories", "--max-items", "100"]),
            runSsmDiagnosticCommand(instance_id, "docker_ps_compact", timeout_seconds),
        ]);
        const repositories = reposResponse.repositories?.filter((repo) => repo.repositoryName?.includes(repository_prefix)).map((repo) => ({
            repositoryName: repo.repositoryName ?? "unknown",
            repositoryUri: repo.repositoryUri ?? "unknown",
        })) ?? [];
        const latestImages = await Promise.all(repositories.map(async (repo) => {
            const imageResponse = await execAwsJson(["ecr", "describe-images", "--repository-name", repo.repositoryName, "--max-items", "20"]);
            const latest = imageResponse.imageDetails
        ...
        const driftReport = runningContainers.map((container) => {
            const matchedRepo = latestImages.find((repo) => container.image.includes(repo.repositoryName) ||
            ...
            return {
                ...
                runtimeTag,
                driftDetected: matchedRepo != null &&
        ...
        const textParts = [
            ...
            ...driftReport.map((item) => `- ${item.containerName}: image=${item.image} repo=${item.matchedRepository ?? "-"} drift=${item.driftDetected ? "yes" : "no"}`),
        ];
        if (runtime.stderr) {
...
server.tool("aws_incident_alb_5xx_snapshot", {
    ...
    try {
        const [loadBalancerResponse, targetGroupResponse, metricCatalog] = await Promise.all([
            execAwsJson(["elbv2", "describe-load-balancers"]),
            execAwsJson(["elbv2", "describe-target-groups"]),
            execAwsJson(["cloudwatch", "list-metrics", "--namespace", "AWS/ApplicationELB"]),
        ]);
        const loadBalancer = loadBalancerResponse.LoadBalancers?.find((item) => item.LoadBalancerName?.includes("bookcabin")) ??
            loadBalancerResponse.LoadBalancers?.[0];
        const targetGroup = targetGroupResponse.TargetGroups?.find((item) => item.TargetGroupName?.includes("bookcabin")) ??
            targetGroupResponse.TargetGroups?.[0];
        const lbDimension = metricCatalog.Metrics?.find((metric) => metric.Dimensions?.some((dimension) => dimension.Name === "LoadBalancer" && dimension.Value?.includes("bookcabin-free-tier-alb")))?.Dimensions?.find((dimension) => dimension.Name === "LoadBalancer")?.Value ?? null;
        const tgDimension = metricCatalog.Metrics?.find((metric) => metric.Dimensions?.some((dimension) => dimension.Name === "TargetGroup" && dimension.Value?.includes("bookcabin-free-tier-tg")))?.Dimensions?.find((dimension) => dimension.Name === "TargetGroup")?.Value ?? null;
        const metricQueries = [];
        if (lbDimension) {
            metricQueries.push(execAwsJson([
                "cloudwatch",
                "get-metric-statistics",
                "--namespace",
                "AWS/ApplicationELB",
        ...
        const [elb5xx, requestCount, target5xx, pathDiagnosis] = await Promise.all([
            ...
            (async () => {
                ...
                const targetHealth = targetGroup?.TargetGroupArn
                    ? await execAwsJson(["elbv2", "describe-target-health", "--target-group-arn", targetGroup.TargetGroupArn])
                    : { TargetHealthDescriptions: [] };
                return {
                    probes,
                    targetHealth: targetHealth.TargetHealthDescriptions?.map((item) => ({
        ...
        const sumDatapoints = (points) => points?.reduce((sum, point) => sum + (point.Sum ?? 0), 0) ?? 0;
        const result = {
            region: awsRegion,
            profile: awsProfile ?? "default",
            minutes,
        ...
        const text = [
            ...
            ...(result.targetHealth.length > 0
                ? result.targetHealth.map((target) => `- Target ${target.targetId}:${target.port ?? "-"} state=${target.state}${target.reason ? ` reason=${target.reason}` : ""}`)
...
server.tool("aws_incident_target_unhealthy_snapshot", {
    target_group_arn: z.string().optional(),
    probe_path: z.string().default("/health"),
    minutes: z.number().int().min(5).max(1440).default(60),
    ...
    try {
        ...
        const targetGroup = (targetGroupArn
        ...
        const targetHealthResponse = await execAwsJson(["elbv2", "describe-target-health", "--target-group-arn", targetGroupArn]);
        const unhealthyTargets = targetHealthResponse.TargetHealthDescriptions?.map((item) => ({
            targetId: item.Target?.Id ?? "unknown",
            port: item.Target?.Port ?? null,
            state: item.TargetHealth?.State ?? "unknown",
            reason: item.TargetHealth?.Reason ?? null,
        ...
        const text = [
            ...
            ...unhealthyTargets.map((target) => `- ${target.targetId}:${target.port ?? "-"} state=${target.state}${target.reason ? ` reason=${target.reason}` : ""}`),
...
server.tool("aws_trace_deployment_source_of_truth", {
    instance_id: z.string().min(4),
    ...
    try {
        ...
        if (repo) {
            try {
                const response = await fetchGitHubApi(`/repos/${repo.slug}/actions/workflows/${encodeURIComponent(workflow_file)}/runs?per_page=5`);
                workflowRuns =
        ...
        const [drift, targetHealth] = await Promise.all([
            (async () => {
                const reposResponse = await execAwsJson(["ecr", "describe-repositories", "--max-items", "100"]);
        ...
        const targetGroupArn = targetHealth.TargetGroups?.find((item) => item.TargetGroupName?.includes("bookcabin"))?.TargetGroupArn ??
            targetHealth.TargetGroups?.[0]?.TargetGroupArn;
        const health = targetGroupArn != null
            ? await execAwsJson(["elbv2", "describe-target-health", "--target-group-arn", targetGroupArn])
            : { TargetHealthDescriptions: [] };
        ...
        const result = {
            ...
            runningContainers,
            targetHealth: health.TargetHealthDescriptions?.map((item) => ({
                targetId: item.Target?.Id ?? "unknown",
                port: item.Target?.Port ?? null,
                state: item.TargetHealth?.State ?? "unknown",
                reason: item.TargetHealth?.Reason ?? null,
        ...
        const text = [
            ...
            ...(workflowRuns.length > 0
                ? workflowRuns.map((run) => `- Run ${String(run.id)} sha=${String(run.headSha ?? "").slice(0, 12)} status=${String(run.status)} conclusion=${String(run.conclusion ?? "-")}`)
...
server.tool("aws_incident_summary", {
    ...
    try {
        ...
        if (scenario === "target_unhealthy") {
            const targetGroupsResponse = await execAwsJson(["elbv2", "describe-target-groups"]);
            const targetGroup = targetGroupsResponse.TargetGroups?.find((item) => item.TargetGroupName?.includes("bookcabin")) ??
                targetGroupsResponse.TargetGroups?.[0];
            return makeTextResult([
...
server.tool("aws_run_ssm_remediation_action", {
    instance_id: z.string().min(4),
    action_id: z.enum(Object.keys(ssmRemediationCommands)),
    ...
    try {
        ...
        return makeTextResult(textParts.join("\n"), {
            ...
            guardrail: "This remediation tool is intentionally narrow and only allows a tiny audited set of restart actions.",
        });
    }
    catch (error) {
...
server.tool("aws_diagnose_delivery_path", {
    probe_path: z.string().default("/health"),
    timeout_seconds: z.number().int().min(3).max(60).default(15),
}, async ({ probe_path, timeout_seconds }) => {
    try {
        const [cloudfrontResponse, loadBalancerResponse, targetGroupResponse] = await Promise.all([
            execAwsJson(["cloudfront", "list-distributions"]),
            execAwsJson(["elbv2", "describe-load-balancers"]),
            execAwsJson(["elbv2", "describe-target-groups"]),
        ]);
        const cloudfrontDistribution = cloudfrontResponse.DistributionList?.Items?.find((item) => item.Comment?.toLowerCase().includes("bookcabin")) ??
            cloudfrontResponse.DistributionList?.Items?.[0];
        const loadBalancer = loadBalancerResponse.LoadBalancers?.find((item) => item.LoadBalancerName?.toLowerCase().includes("bookcabin")) ??
            loadBalancerResponse.LoadBalancers?.[0];
        const targetGroup = targetGroupResponse.TargetGroups?.find((item) => item.TargetGroupName?.toLowerCase().includes("bookcabin")) ??
            targetGroupResponse.TargetGroups?.[0];
        const targetHealth = targetGroup?.TargetGroupArn
            ? await execAwsJson(["elbv2", "describe-target-health", "--target-group-arn", targetGroup.TargetGroupArn])
            : { TargetHealthDescriptions: [] };
        ...
        const text = [
            ...
            ...(probes.length > 0
                ? probes.map((probe) => `- Probe ${probe.label}: ${probe.status} ${probe.statusText} (${probe.elapsedMs}ms) -> ${probe.finalUrl}`)
...
server.prompt("plan_uiux_change", {
    request: z.string(),
}, ({ request }) => ({
    messages: [
        {
